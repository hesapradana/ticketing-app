# Module 13 : Detail Event

Pada modul ini kita akan membuat **halaman detail event**, yaitu halaman yang menampilkan informasi lengkap sebuah event beserta daftar tiket yang bisa dibeli oleh user.

Halaman ini menjadi **jembatan utama** antara user dan proses pembelian tiket.
...
```bash
php artisan make:controller User/EventController
```

Edit file: `app/Http/Controllers/User/EventController.php`
...
- Method `show(Event $event)` menggunakan **Route Model Binding**
    - Laravel otomatis mengambil data event berdasarkan `{event}` di URL
- `$event->load([...])` digunakan untuk **eager loading**
    - `tikets` → daftar tiket event
    - `kategori` → kategori event
    - `user` → penyelenggara event
- Tujuan eager loading:
    - Menghindari **N+1 query problem**
    - Performa lebih baik saat data ditampilkan di view
- Data event dikirim ke view `events.show`

## **Setup Route Dashboard**

Edit file `routes/web.php`, tambahkan route untuk detail event:
...
![image.png](attachment:b6a23df6-a843-472e-bb8d-5e9d7653eecd:image.png)
...
1. Auth
    ...
    - `@auth`
        - Hanya user yang **sudah login** yang bisa checkout
    - `@else`
        - Jika belum login, user diarahkan ke halaman login
2. Update Ringkasan Pembelian
    ...
    - Menghitung total tiket yang dipilih
    - Menghitung total harga
    - Meng-update:
        - Jumlah item
        - Total pembayaran
        - Daftar tiket terpilih
    - Mengaktifkan / menonaktifkan tombol checkout
3. Tombol Tambah & Kurang Tiket
...
Tambahkan `:href="route('events.show', $event)" />`

### **Penjelasan**

- Properti `href` ditambahkan agar:
    - Setiap card event bisa diklik
    - User diarahkan ke halaman detail event
- Menggunakan `route('events.show', $event)`
    - Lebih aman
    - Otomatis mengikuti perubahan route

## Testing Show Detail Event

1. **Akses Halaman Detail Event**
    - Buka halaman **Homepage**
    - Klik salah satu **card event**
    - Pastikan URL berubah menjadi:
        
        ```
        /events/{id}
        ```
        
2. **Pastikan Informasi Event Tampil**
    
    Periksa apakah data berikut muncul dengan benar:
    ...
    Jika semua data tampil sesuai dengan data di database, maka **load data event berhasil**.
    
    ![image.png](attachment:423fdde7-d450-43a3-8e42-2c27d230a4de:image.png)
    
3. **Cek Daftar Tiket**
    - Pastikan daftar tiket tampil di halaman
    - Untuk setiap tiket, periksa:
        - Nama / tipe tiket
        - Harga tiket (atau “Gratis” jika harga 0)
        - Stok tiket
        - Input jumlah tiket
    
    Jika event belum memiliki tiket, pastikan muncul pesan:
    
    > Tiket belum tersedia untuk acara ini.
    > 
    
    ![image.png](attachment:58c253aa-988f-4514-9be9-be3db8f4774a:image.png)
    
4. **Uji Interaksi Tambah & Kurang Tiket**
    - Klik tombol **( + )** untuk menambah jumlah tiket
    - Klik tombol **( − )** untuk mengurangi jumlah tiket
    - Pastikan:
        - Jumlah tidak bisa kurang dari 0
        - Jumlah tidak bisa melebihi stok
        - Subtotal tiket berubah sesuai jumlah
    
    ![image.png](attachment:f4597d71-d850-4814-ae11-2482ab3a7daf:image.png)
    
5. **Uji Status Login**
    
    **Belum Login**
    
    - Pastikan tombol **Login untuk Checkout** tampil
    - Tombol **Checkout** tidak tersedia
    
    ![image.png](attachment:087282b9-7ad0-4c09-b85d-ad1e7ab412c2:image.png)
    
    **Sudah Login**
    
    - Pastikan tombol **Checkout** muncul
    - Tombol **Checkout** aktif hanya jika minimal 1 tiket dipilih