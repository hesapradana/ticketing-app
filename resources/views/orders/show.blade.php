<x-layouts.app>
  <section class="max-w-4xl mx-auto py-12 px-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="text-sm text-gray-500">Order #{{ $order->id }} - {{ $order->order_date->translatedFormat('d F Y, H:i') }}</div>
    </div>
          ...
          <div class="space-y-3">
            @foreach($order->detailOrders as $detail)
              <div class="flex justify-between items-center">
                <div>
                  <div class="font-bold">{{ $detail->tiket->ticketType->nama }}</div>
                  <div class="text-sm text-gray-500">Qty: {{ $detail->jumlah }}</div>
                </div>
                <div class="text-right">
                  <div class="font-bold">Rp {{ number_format($detail->subtotal_harga, 0, ',', '.') }}</div>
                </div>
              </div>
            @endforeach
          </div>
